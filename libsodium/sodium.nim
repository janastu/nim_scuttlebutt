...
type
  ...
  SecretBoxMac* = array[crypto_secretbox_MACBYTES, char]
...
proc crypto_secretbox_easy*(msg: string,
                            nonce: SecretBoxNonce,
                            key: SecretBoxKey): string =
...
proc crypto_secretbox_detached(
  c: ptr cuchar,
  mac: ptr cuchar,
  m: ptr cuchar,
  mlen: csize,
n: ptr cuchar,
k: ptr cuchar):cint {.sodium_import.}

proc crypto_secretbox_detached*(key: SecretBoxKey,
                                nonce: SecretBoxNonce,
                                msg: string): (string, SecretBoxMac) =
  ## Encrypt + sign a variable len string with a preshared key
  result[0] = newString msg.len
  let
    c_ciphertext = cpt result[0]
    cmac = cpt result[1]
    cmsg = cpt msg
    mlen = cpsize msg
    cnon = cpt nonce
    ckey = cpt key
  let rc = crypto_secretbox_detached(c_ciphertext, cmac, cmsg, mlen, cnon, ckey)
  check_rc rc
...
proc crypto_secretbox_open_easy*(ciphertext: string,
                                 nonce: SecretBoxNonce,
                                 key: SecretBoxKey): string =
...
proc crypto_secretbox_open_detached(
  decrypted: ptr cuchar,
  c: ptr cuchar,
  mac: ptr cuchar,
  clen: csize,
...
proc crypto_secretbox_open_detached*(key: SecretBoxKey,
                                     nonce: SecretBoxNonce,
                                     mac: SecretBoxMac,
  ...
  let
    ...
    c_nonce = cpt nonce
    c_key = cpt key
  let rc = crypto_secretbox_open_detached(
    c_decrypted,
    c_ciphertext,
    c_mac,
    ciphertext_len,
...
assert(crypto_box_PUBLICKEYBYTES == crypto_scalarmult_curve25519_BYTES)
assert(crypto_box_SECRETKEYBYTES == crypto_scalarmult_curve25519_BYTES)
type
  BoxPublicKey* = Curve25519Pk
  BoxSecretKey* = Curve25519Sk
  BoxNonce* = array[crypto_box_NONCEBYTES, char]
  BoxSeed* = array[crypto_box_SEEDBYTES, char]
  BoxPair* = tuple[pub: BoxPublicKey, sec: BoxSecretKey]
...
proc crypto_box_keypair*(): BoxPair =
  let
    pk = cpt result[0]
    sk = cpt result[1]
...
proc crypto_box_seed_keypair*(seed: BoxSeed): BoxPair =
  let
    pk = cpt result[0]
    sk = cpt result[1]
    seed = cpt seed
...
type
  SignPublicKey* = Ed25519Pk
  ## Ed25519 public key.
  SignSecretKey* = Ed25519Sk
  ## Ed25519 secret key.
  SignDetached* = array[crypto_sign_BYTES, char]
  SignSeed* = array[crypto_sign_SEEDBYTES, char]
  SignPair* = tuple[pub: SignPublicKey, sec: SignSecretKey]
...
proc crypto_sign_keypair*(): SignPair =
...
proc crypto_sign_seed_keypair*(seed: SignSeed): SignPair =
...
proc crypto_sign_detached*(message: string,
                           secret_key: SignSecretKey): SignDetached =
...
proc crypto_sign_verify_detached*(signature: SignDetached,
                                  message: string,
                                  public_key: SignPublicKey): bool =
  ## Verify that a signature is a valid for the given message and key.
  let
    pk = cpt public_key
    sig = cpt signature
    msg = cpt message
